...
{
    public class CRMBulkInstagramLeads
    {
        public void UpdateCRMLeadStatusForBulkInstagramLeads(DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                //var dbData = (from fbleads in db.CRMInstagramLeads
                ...
                var dbData = db.CRMInstagramLeads
                ...
                {
                    LogException.Log("BergerLeadCRMSchedular.DB BergerLeadCRMSchedular.DB CRMBulkInstagramLeads.UpdateCRMLeadStatusForBulkInstagramLeads :  Not data found in Database.");
                    return;
                }
                ...
                foreach (var getlist in dbData)
                {
                    ...
                    try
                    {
                        ...
                        var Instagramleadquery = db.CRMInstagramLeads
                                            .Where(d => d.CRMLeadId == leadDetail.Id)
                                            .Select(d => d)
                                            .FirstOrDefault();

                        if (Instagramleadquery != null)
                        {
                            ...
                            Instagramleadquery.CRMLeadSubStatusId = CRMLeadSubStatusId;
                            ...
                            Instagramleadquery.CRMRecordId = _data.Id;
                            Instagramleadquery.CRMPincode = _data.PinCode;
                            Instagramleadquery.CRMLeadStatusUpdatedOn = DateTime.Now;
                            try
                            {
                                db.SaveChanges();
                            }
                            ...
                            {
                                LogException.Log(string.Format("{0},{1},{2},{3}", "Exception in BergerLeadCRMSchedular.DB CRMBulkInstagramLeads.UpdateCRMLeadStatusForBulkInstagramLeads : ", status, leadDetail.Id, ex.Message));
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        LogException.Log(string.Format("CRMBulkInstagramLeads splitData exception : .{0}", ex.Message));
                    }
            ...
            {
                LogException.Log(string.Format("{0},{1}", "Exception in BergerLeadCRMSchedular.DB CRMBulkInstagramLeads.UpdateCRMLeadStatusForBulkInstagramLeads : ", ex.Message));
            }

            LogException.Log("CRMBulkInstagramLeads Updated.");
        }
    }
}
